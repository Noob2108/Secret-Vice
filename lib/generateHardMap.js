...
// --- modify function signature (add startOffsetBeats) ---
export function generateHardMap(analysis, options = {}) {
  ...
  // NEW: lead-in (beats) before first block appears (default 4 beats)
  ...
  for (let i = 0; i < beatFeatures.length; i += 4) {
    ...
    // Soften first bar: fewer notes and no staccato bursts in bar 0
...
if (isFirstBar) target = Math.min(target, 2);
    ...
    let motifName;
if (isFirstBar) {
  motifName = "altDiag2"; // gentle opener
