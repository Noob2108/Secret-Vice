...
// Secret Vice — Hard generator v6.1 (flow-first with rebound alternation)
...
const FOLLOW={0:[5,4,8],1:[2,3,8],2:[5,1,8],3:[4,0,8],4:[0,3,8],5:[0,2,8],6:[1,3,8],7:[1,2,8],8:[5,4,2,3]};
// rebound pairs + families
const OPP = {0:1,1:0,2:3,3:2,4:7,7:4,5:6,6:5,8:8};
const DIR_FAMILY = d => (d===0||d===1?"vert":d===2||d===3?"horiz":d>=4&&d<=7?"diag":"dot");
...
export function generateHardMap(analysis, options={}){
  const preset=options.preset||"dancey";
  ...
  const map={version:"3.3.0",bpmEvents:[],rotationEvents:[],colorNotes:[],sliders:[],burstSliders:[],obstacles:[],basicBeatmapEvents:[],colorBoostBeatmapEvents:[]};
  ...
  const sectionLabel=(bar)=>{const center=bar.reduce((a,b)=>a+b.timeSec,0)/bar.length;const e=classifyEnergy(onsets,center);if(e==="high")return"chorus";if(e==="mid")return"verse";return"intro";};

  const palettes={intro:{density:[1,2],dotChance:0.2,diagChance:0.6,subdiv:4},verse:{density:[2,3],dotChance:0.15,diagChance:0.7,subdiv:4},chorus:{density:[3,4],dotChance:0.1,diagChance:0.85,subdiv:8}};
  ...
  let baseX=1, baseY=1;
  const clampX=(x)=>clamp(x,0,3), clampInnerX=(x)=>clamp(x,1,2), clampY=(y)=>clamp(y,0,2);

  for(let bi=0;bi<bars.length;bi++){
    const bar=bars[bi]; if(!bar.length) continue;
    const label=sectionLabel(bar), pal=palettes[label];
    ...
    for(let j=0;j<bar.length;j++){
      const bf=bar[j], b0=secToBeats(bf.timeSec)+startOffsetBeats, count=perBeat[j];
      baseX = clampInnerX(baseX + (rand()<0.5?-1:+1));
      baseY = clampY(baseY + (rand()<0.4?0:(rand()<0.5?-1:+1)));

      let placed=0, offset=0;
      const stepGap = pal.subdiv===8?0.125:0.25;

      while(placed<count){
        const last = map.colorNotes[map.colorNotes.length-1];
        const useDiag=rand()<pal.diagChance;
        let suggest=useDiag?(leadLeft?(rand()<0.5?5:7):(rand()<0.5?4:6)):(rand()<0.5?0:1);
        let d=pickDirFollow(lastDir,suggest);

        // rebound alternation
        if(last && (b0-last.b)<0.5){
          const sameFam=DIR_FAMILY(d)===DIR_FAMILY(last.d)&&d!==8&&last.d!==8;
          if(sameFam) d=OPP[last.d];
        }

        const c=leadLeft?0:1;
        let x=clampInnerX((last?(last.x+(leadLeft?-1:+1)):baseX));
        ...
        const tooClose=prev&&(nb-prev.b)<0.18;
        const sameCell=prev&&prev.x===x&&prev.y===y&&(nb-prev.b)<0.35;
        if(tooClose||sameCell){offset+=stepGap;continue;}
