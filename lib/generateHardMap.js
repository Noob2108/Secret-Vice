...
// Secret Vice — Hard generator v5
// - Lead-in (startOffsetBeats)
// - Density smoothing per bar
// - Swing planner + sightline safety
// - Repetition guard
// - Lighting packs with custom colours (Chroma-friendly)
...
//////////////// flow ////////////////
const FLOW_FOLLOW = {
  0: [5, 4, 8], 1: [2, 3, 8], 2: [5, 1, 8], 3: [4, 0, 8],
  4: [0, 3, 8], 5: [0, 2, 8], 6: [1, 3, 8], 7: [1, 2, 8],
  8: [5, 4, 2, 3]
};
...
function sightlineSafe(lastNotes, candidate, horizonBeats = 0.5) {
  for (let i = lastNotes.length - 1; i >= 0; i--) {
    ...
    if (n.x === candidate.x && n.y === candidate.y) {
      if (!(n.d === 8 || candidate.d === 8)) return false;
    }
  }
  return true;
}
...
export function generateHardMap(analysis, options = {}) {
  const preset = options.preset || "tech";
  const lights = options.lights || "concert";
  const colors = options.colors || { primary: "#ff2aa0", secondary: "#6a5cff" };

  // lead-in beats
  const startOffsetBeats = Number.isFinite(options.startOffsetBeats)
    ? Math.max(0, options.startOffsetBeats)
    : 4;
  ...
  const styleMotifs = preset === "dancey"
    ...
    : preset === "showpiece"
      ...
      : ["altDiag2", "jabReset2", "burst3", "inOut3"];
  ...
  for (let i = 0; i < beatFeatures.length; i += 4) {
    ...
    if (isFirstBar) target = Math.min(target, 2);

    // distribute placements per beat
    ...
    for (let j = 0; j < bar.length; j++) {
      const bf = bar[j];
      let b = secToBeats(bf.timeSec) + startOffsetBeats;
      ...
      if (isFirstBar) {
        ...
        motifName = "staircase3";
      } else {
        motifName = styleMotifs[Math.floor(rand()*styleMotifs.length)];
      }
    ...
    const bStart = secToBeats(bar[0].timeSec) + startOffsetBeats;
